...
def Caesar_encrypt(PT,Key):
    return ''.join([chr((ord(item)+Key-65)%26+65) for item in PT])
    # use the related askII code to calculate

def Caesar_decrypt(CT,Key):
    return ''.join([chr((ord(item)-Key-65)%26+65) for item in CT])
...
def Matrix_transposition_encrypt(PT,Key):
    gen_String = [i for i in PT.ljust(len(PT)+len(Key)-len(PT)%len(Key)).replace(' ','%')]
    #fill space with %
    gen_Matrix = numpy.array(gen_String).reshape(int(len(gen_String)/len(Key)),len(Key)).T
    # map the string into matrix and return its transpose
    return ''.join([''.join(gen_Matrix[int(i)-1]) for i in Key])
